...
class SubmissionInviteView(View):
    ...
    def post(self, request, activity_uuid):
        ...
        submission.save(
            update_fields=["activity_status", "activity_start_time", ])
...
class SubmitSolutionView(View):
    ...
    def post(self, request, activity_uuid):
        ...
        if submission is None:
            ...
            messages.error(
                request, 'Solution not submitted.You have already submitted a solution before.')
            return HttpResponseRedirect(reverse('submission_invite', args=(submission.activity_uuid,)))

        submission.activity_status = ActivityStatus.SUBMITTED.value
        submission.activity_solution_link = input_solution_link
        submission.save(
            update_fields=["activity_status", "activity_solution_link", ])

        send_emails.delay(submission.activity_uuid,
                          EmailType.ACTIVITY_SOLUTION.value)
