...
class TestTasks(TestCase):
    ...
    def test_checkout_pending_tas_to_send_reminders_to_start_mails(self, mocked_send_emails):
        ...
        for day in range(1, 7):
            submission = Submission.objects.create(
                invitation_creation_dateandtime=timezone.now()-datetime.timedelta(days=day))
            EmailLog.objects.create(activity_uuid=submission.activity_uuid,
                                    date_of_mail=submission.invitation_creation_dateandtime, mail_status='SENT')
    ...
    def test_checkout_pending_task_to_send_reminder_to_submit_mails(self, mocked_send_emails):
        submission = Submission.objects.create(
            invitation_creation_dateandtime=timezone.now()-datetime.timedelta(days=1, hours=22))
        EmailLog.objects.create(activity_uuid=submission.activity_uuid,
                                date_of_mail=submission.invitation_creation_dateandtime, mail_status='SENT')
    ...
    def test_checkout_pending_task_to_send_expiry_mails_to_admin(self, mocked_send_emails):
        ...
        self.assertEqual(mocked_send_emails.call_count, 2)
        self.assertEqual(submission.activity_status,
