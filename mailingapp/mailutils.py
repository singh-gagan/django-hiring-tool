...
class MailUtils:
    ...
    def create_messages(cls, submission, email_type):
        ...
        if (email_type == EmailType.ACTIVITY_EXPIRED.value or
                email_type == EmailType.ACTIVITY_SOLUTION.value):
    ...
    def create_mail_body(cls, submission, email_type, message):
        ...
        mail_body_keywords = {
            ...
            EmailTemplatePlaceholder.ACTIVITY_URL.value: (local_settings.HOST +
                                                          reverse('submission_invite',
                                                                  args=(submission.activity_uuid,))),

            EmailTemplatePlaceholder.ACTIVITY_START_TIME.value: ("" if submission.activity_start_time is None
                                                                 else submission.activity_start_time.astimezone(
                                                                     user_time_zone).strftime("%Y-%m-%d %H:%M:%S")),

            EmailTemplatePlaceholder.ACTIVITY_SOLUTION_LINK.value: submission.activity_solution_link,

            EmailTemplatePlaceholder.ACTIVITY_LEFT_TIME.value: ("" if submission.activity_left_time is None
                                                                else (str(submission.activity_left_time.days)+" days "+str(
                                                                    submission.activity_left_time.seconds//3600)+" hours " +
                                                                    str((submission.activity_left_time.seconds//60) % 60)+" minutes "))
        }
