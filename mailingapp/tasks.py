...
def send_emails(activity_uuid, email_type):
    ...
    logger.info('Sending {email_type} email. Actiivty UUID-{activity_uuid}'.format(
    ...
    try:
        ...
        logger.error(
            '{} email not sent. Activity UUID - {}'.format(email_type, activity_uuid), exc_info=e)
        return

    sent_message = GmailServices.send_message(
        submission.invitation_host, message)
...
def send_reminder_to_start_email(submission, reminders_gap_list):
    ...
    if gap.days in reminders_gap_list:
        send_emails.delay(submission.activity_uuid,
                          EmailType.START_REMINDER.value)
...
def send_reminder_to_submit_email(submission):
    ...
    if timezone.now() >= activity_reminder_time:
        send_emails.delay(submission.activity_uuid,
                          EmailType.SUBMISSION_REMINDER.value)
...
def send_activity_expired_email(submission):
    ...
    send_emails.delay(submission.activity_uuid,
                      EmailType.ACTIVITY_EXPIRED.value)
